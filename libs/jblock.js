...
var jBlockNode =
{
    ...
    map: function (selector, fn)
    {
        var currentCtx = this._ctx
            elems = this.copy(selector),
            result = []
    ...
    copy: function ()
    {
        ...
        for (var i = 0, ii = this._ctx._content.length; i < ii; i++)
        {
            ...
            if (typeof child === 'string' || typeof child === 'undefined' || child === null)
                continue
            ...
            if (Array.prototype.indexOf.call(arguments, '!'+ childName) >= 0)
                continue

            if (hasNotSelector || Array.prototype.indexOf.call(arguments, childName) >= 0)
                result.push(child)
        }
